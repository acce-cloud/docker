...
docker service create --replicas 1 --name filemgr -p 9000:9000 -p 8983:8983 --network swarm-network  --constraint 'node.labels.ecostress_type==head_node'\
                      --mount type=bind,src=/usr/local/adeploy/archive,dst=/usr/local/oodt/archive oodthub/oodt-filemgr
...
docker service create --replicas 1 --name wmgr -p 9001:9001 --network swarm-network --constraint 'node.labels.ecostress_type==head_node'\
                      ...
                      --env 'FILEMGR_URL=http://filemgr:9000/' oodthub/oodt-example-ecostress 
