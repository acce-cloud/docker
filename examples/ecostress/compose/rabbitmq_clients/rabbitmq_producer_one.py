...
class RabbitmqProducerOne(object):
    ...
    def publish(self, run_number, task_number, pge_number, msg_dict):
...
def main(workflow_event, run_number, task_number, pge_number, msg_dict):
    ...
    rmqProducer = RabbitmqProducerOne(rabbitmqUrl + '?connection_attempts=3&heartbeat_interval=3600', workflow_event)
    
    # publish message
    rmqProducer.publish(run_number, task_number, pge_number, msg_dict)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
      raise Exception("Usage: python rabbitmq_producer_one.py <workflow_event> <run> <task> <pge> [<metadata_key=metadata_value> <metadata_key=metadata_value> ...]")
    else:
      workflow_event = sys.argv[1]
      run_number = int( sys.argv[2] )
      task_number = int( sys.argv[3] )
      pge_number = int( sys.argv[4] )
    ...
    main(workflow_event, run_number, task_number, pge_number, msg_dict)
